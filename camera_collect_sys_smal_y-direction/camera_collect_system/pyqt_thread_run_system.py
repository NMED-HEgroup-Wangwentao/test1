...
class MyThread_save_image(QThread):
    def __init__(self,data,filename,path):
        ...
        if q.start_save_buttom==1:
            ...
            if q.start_save_buttom == 1:
                if q.start_save_weiyitai == 1:
                    ...
                    # try:
                    #     q.filename=self.filename
                    #     q.f=open(self.path+'/'+str(q.filename) + ".bin", 'ab')
                    # except:
...
class App(QDialog):
    ...
    def __init__(self,user):
        ...
        # self.wave_change_laser=wave()
        # self.wave_change_laser.create()
        # t=self.pool.submit(self.wave_change_laser.sent_wave)
        self.wave_change_laser=wave()
        self.wave_change_laser.create()
        t=self.pool.submit(self.wave_change_laser.sent_wave)
    ...
    def paint(self):
        ...
        self.label = QLabel()
        self.label_1=QLabel()
        self.image_box.setLayout(self.layout)
    ...
    def create_hist(self):
        ...
        #layout_1.addWidget(self.widgets["image"][0].slider_min, 0, 0, 1, 1)
        ...
        if self.pco_save_flag == False:
            try:
                if self.pco_control.start_flag:
                    ...
                    try:
                        q.filename=self.widgets["schedule_bar"][1].text()
                        q.f=open(self.f_path+'/'+str(q.filename) + ".bin", 'ab')
                        print("self.f_path")
                        print(self.f_path)
                    except:
                        q.filename = self.widgets["schedule_bar"][1].text()
    ...
    def get_m_time(self):
        try:
            ...
            t1 = math.ceil((z_distance_end - z_distance_start)*20 / 0.4)
    ...
    def draw_image(self):
        while self.pco_control.start_flag:
            ...
            # t1.start()
            # t2.start()
            self.save_image_thread=MyThread_save_image(self.np_image,self.widgets["schedule_bar"][1].text(),self.f_path)
    ...
    def start_displacement_system(self):
        ...
        print("zhunbei ")
        # self.displacement_control_system.set_axial_speed(1,50)
        # self.displacement_control_system.set_axial_speed(2, 50)
        # self.displacement_control_system.set_axial_speed(3, 50)
        #
    ...
    def on_update_button_click_start(self):
        ...
        if self.displacement_control_system.displacement_start_flag == True:
            ...
            try:
                ...
                print("z_distance_start是空")
                z_distance = float(str(self.displacement_control_system.query_position("z_position"))[0:-2]) * 0.05
                print("z是空的就不采集了")
                q.start_save_buttom = 0
                q.start_save_weiyitai = 0

            self.displacement_control_system.set_axial_speed(1, 3)
            self.displacement_control_system.set_axial_speed(2, 3)
            self.displacement_control_system.set_axial_speed(3, 3)
    ...
    def on_update_button_click_end(self):
        ...
        if self.displacement_control_system.displacement_start_flag == True:
            ...
            self.displacement_control_system.set_axial_speed(2, 0.24)
            self.displacement_control_system.set_axial_speed(3, 0.24)
