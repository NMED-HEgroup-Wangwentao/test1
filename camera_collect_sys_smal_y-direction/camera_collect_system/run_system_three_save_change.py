...
class App(QDialog):
    ...
    def __init__(self, user):
        ...
        self.draw_image_stop_flag=False
        self.save_stop_image_flag=False
    ...
    def save_collect_clicked(self):
        print("调用了保存采集")
        print(self.pco_save_flag)
        self.draw_image_stop_flag=True
        self.draw_thread.join()
        self.save_image_thread=threading.Thread(target=self.save_image)
        self.save_image_thread.start()
        ...
        if self.pco_save_flag == False:
            try:
                ...
                self.set_final_schedule = 1
                self.save_stop_image_flag=True
                self.save_image_thread.join()
                self.displacement_get_position = threading.Thread(target=self.get_position_thread)
                print("zhunbei ")
                self.displacement_get_position.start()
        ...
        while True:
            ...
            if q.start_save_buttom == 1:
                ...
                if q.start_save_weiyitai == 0:
                    ...
                    # print(time.asctime(time.localtime(time.time())))
                    try:
                        self.array_to_save[self.array_to_save_number, :, :] = self.pco_control.np_image
                        # print(self.np_image)
            ...
            if q.start_save_buttom == 1:
                if q.start_save_weiyitai == 1:
                    ...
                    q.i = q.i + 1
                    # q.f = open("C:\\TEST\\" + str(q.filename) + "_" + str(q.i) + ".bin", 'ab')
            if self.save_stop_image_flag==True:
                self.save_stop_image_flag=False
                break
    ...
    def draw_image(self):
        ...
        while self.pco_control.start_flag:
           ...
           # self.np_image = copy.deepcopy(self.pco_control.np_image)
            ...
            if draw_frq == 50:
                draw_frq = 0
            if self.draw_image_stop_flag==True:
                self.draw_image_stop_flag=False
                break
    ...
    def on_update_button_click_start(self):
        ...
        if self.displacement_control_system.displacement_start_flag == True:
            ...
            self.displacement_control_system.set_axial_speed(1, 20)
            self.displacement_control_system.set_axial_speed(2, 50)
            self.displacement_control_system.set_axial_speed(3, 2)

            self.displacement_control_system.chuansong_1(self.x_distance_for_save+10)
            self.displacement_control_system.chuansong_3(self.z_distance_for_save+10)

            self.displacement_control_system.chuansong_2(y_distance+10)
            self.displacement_control_system.chuansong_start_flag(1)
            # self.displacement_control_system.update_move_and_query(1)
            ...
            # self.displacement_control_system.chuansong_1(x_distance)
            # self.displacement_control_system.chuansong_2(y_distance)
            self.displacement_control_system.set_axial_speed(1, 0.04)
            self.displacement_control_system.set_axial_speed(2, 0.04)
            self.displacement_control_system.set_axial_speed(3, 0.04)
    ...
    def on_update_button_click_end(self):
        ...
        if self.displacement_control_system.displacement_start_flag == True:
            ...
            self.displacement_control_system.set_axial_speed(2, 20)
            ...
            time_end = (int(self.widgets["displacement_control"][13].text()) - int(
                self.widgets["displacement_control"][10].text()))  / 1000
            time.sleep(abs(time_end +0.2))
